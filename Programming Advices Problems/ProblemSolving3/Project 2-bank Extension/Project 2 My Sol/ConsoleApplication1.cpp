...
{
    cout << "Choose what do you want to do? [1 to7]? ";
    short Choice = 0;
    cin >> Choice;
    return Choice;
}
...
void PerfromMainMenueOption(enMainMenueOptions MainMenueOption)
{
    ...
    case enMainMenueOptions::Transaction_Option:
        system("cls");
        Transaction_Menu_Screen();
        break;
...
stClient Edit_Client_Balance(double Edit_Amount, stClient &ClientData)
{
    ClientData.AccountBalance += Edit_Amount;
    return ClientData;
}

bool Has_Enough_Credit(double Amount, stClient Balance)
{
    return Balance.AccountBalance >= Amount;
    
}

bool UpdateClientBalance(string AccountNumber, vector<stClient>& vClients, double Deposit_Amount)
{
    ...
    if (FindClientByAccountNumber(AccountNumber, vClients, Client))
    {
        ...
        if (Answer == 'y' || Answer == 'Y')
        {
            for (stClient& C : vClients)
            {
                if (C.Account_Number == AccountNumber)
                {
                    C = Edit_Client_Balance(Deposit_Amount, C);
                    break;
                }
            }
...
void PrintClientName_With_Balnce(stClient Client)
{
...
void TotalBalances_Screen()
{
    ...
    if (vClients.size() == 0)
        ...
        for (stClient Client : vClients)
        {
            ...
            PrintClientName_With_Balnce(Client);
            cout << endl;
        }
...
{
    TotalBalances_Screen();
    cout << "\n\n \t\t Total Balances is : " << Sum_Of_Balances() << endl;
}
...
void Start_Deposit()
{
    ...
    while (!FindClientByAccountNumber(Account_Number, vClients, Clients))
    {
    ...
    UpdateClientBalance(Account_Number, vClients, Amount);
}
...
void StartWithDraw()
{
    ...
    while (Amount > Clients.AccountBalance)
    {
    ...
    UpdateClientBalance(Account_Number, vClients, Amount * -1);
}

void GoBackToTransaction_Menue()
{
...
void Transactions(enTransactions User_Option)
{
    switch (User_Option)
    {
...
void Transaction_Menu_Screen()
{
    system("cls");
    cout << "===========================================\n";
    cout << "\t\tMain Transaction Menu Screen\n";
    cout << "===========================================\n";
    cout << "\t[1] Deposit.\n";
    cout << "\t[2] WithDraw.\n";
    cout << "\t[3] Total Balanes.\n";
    cout << "\t[4] Main_Menu.\n";
    cout << "===========================================\n";
    Transactions(enTransactions(ReadTransactionOption()));
}
int main()
