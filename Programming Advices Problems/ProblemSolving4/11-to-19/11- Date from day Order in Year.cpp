...
stDate Date_From_Day_Order_in_Year(short Number_Of_Days, short Year)
{
    stDate Date;
    Date.Year = Year;
...
int main()
{
    ...
    short Days_From_Year_Beginning = Days_From_Beginning(Day, Month, Year);
    stDate Date = Date_From_Day_Order_in_Year(Days_From_Year_Beginning, Year);
    printf("Number of Days From The Beginning of the year is : %d\n", Days_From_Year_Beginning);

    printf("Date for [%d] is : %d/%d/%d ", Days_From_Year_Beginning, Date.Day, Date.Month, Date.Year);
    return 0;
}
